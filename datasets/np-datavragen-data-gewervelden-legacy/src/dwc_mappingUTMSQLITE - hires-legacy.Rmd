...
subtitle: "VBP-INBO-waarnemingen-occurrences_202_gewervelden_hoge resolutie_legacy
...
```{r eval=FALSE, include=FALSE}
input_interim <- dbReadTable(con, "INBO_WNMN_CHORDATA_legacy")  # werkt ook voor views
```
...
```{r}
occurrence %<>% mutate(dwc_datasetID = "https://doi.org/10.15468/kxsxxx")
```
...
```{r}
occurrence %<>% mutate(dwc_datasetName = "INBO collaborator Occurrences -High resolution - of vertebrates recorded in waarnemingen.be in Flanders, Belgium, legacy dataset") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
...
DISTINCT NEEDED, don't know why
```{r}
occurrence <- occurrence %>% distinct()
```
...
```{r}
write_csv(occurrence, here::here("datasets", "np-datavragen-data-gewervelden-legacy", "data", "processed", "occurrence_vertebrates_high_res_legacy.csv"), na = "")
```
