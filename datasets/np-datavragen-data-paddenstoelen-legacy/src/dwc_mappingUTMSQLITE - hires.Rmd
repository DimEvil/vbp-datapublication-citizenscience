...
subtitle: "VBP-INBO-waarnemingen-occurrences_paddenstoelen-legacy
...
```{r eval=FALSE, include=FALSE}
input_interim <- dbReadTable(con, "INBO_WNMN_FUNGI_legacy")  # werkt ook voor views
```
...
```{r}
occurrence %<>% mutate(dwc_datasetName = "INBO collaborator Occurrences of Fungi recorded in waarnemingen.be in Flanders, Belgium (Hoge resolutie, legacy)") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
...
occurrence %<>%
  ...
  mutate(verbatimcoordinatesystem = "WGS84") %>%
...
```{r}
write_csv(occurrence, here::here("datasets", "np-datavragen-data-paddenstoelen-legacy", "data", "processed", "occurrence_fungi_high_res.csv"), na = "")
```
