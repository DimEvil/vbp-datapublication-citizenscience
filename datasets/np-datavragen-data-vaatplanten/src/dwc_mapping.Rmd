...
```{r}
input_interim <- read.csv(here::here("vbp-datapublication-citizenscience","datasets", "np-datavragen-data-planten", "data", "interim", "occurrence_planten_inbo_wnmn.csv" ))
```
...
```{r}
occurrence %<>% mutate(dwc_datasetID = "placeholder")
```
...
```{r}
occurrence %<>% mutate(dwc_datasetName = "VBP - INBO collaborator Occurrences of plants recorded in waarnemingen.be in Flanders, Belgium") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
...
```{r}
occurrence %<>% mutate(dwc_occurrenceID = str_c("VBP:OCC:WNMN:", occurrenceID))
...
```{r}
occurrence %<>% mutate(dwc_occurrenceRemarks = notes)
```
...
```{r}
write_csv(occurrence, here::here("datasets", "np-datavragen-data-planten", "data", "processed", "occurrence_planten.csv"), na = "")
```
