...
read_and_add_filename <- function(file) {
  ...
  observerN$filename <- basename(file)  # Volledige bestandsnaam toevoegen als kolom
  observerN
}
```
...
# Lijst van CSV-bestanden ophalen
```{r eval=FALSE, include=FALSE}
...
# Functie om een bestand in te lezen en het relevante deel van de bestandsnaam toe te voegen
```{r}
read_and_add_filename <- function(file) {
  df <- read_csv(file)
