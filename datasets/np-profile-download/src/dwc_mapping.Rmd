...
```{r}
occurrence %<>% mutate(dwc_rightsHolder = str_c(str_to_title(Extracted) , " INBO ")) # e.g. "INBO"
```
### accessRights
...
```{r}
occurrence %<>% mutate(dwc_occurrenceID = str_c("VBP:OCC:WNMN", id , ":", link))
...
occurrence %<>% 
    ...
    mutate(occurrenceStatus = case_when(dwc_individualCount > 0 ~ "Present",
