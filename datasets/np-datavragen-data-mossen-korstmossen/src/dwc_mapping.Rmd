...
subtitle: "VBP-INBO-waarnemingen-occurrences_202_mossen_wieren_native
...
```{r eval=FALSE, include=FALSE}
input_interim <- read.csv(here::here("datasets", "np-datavragen-data-mossen", "data", "interim", "occurrence_planten_inbo_wnmn.csv" ))
```
...
Define which group of species to map to DwC (DOn't do this, filtering done in sqlite)
...
```{r}
occurrence %<>% mutate(dwc_datasetName = "INBO collaborator Occurrences of Bryophyta, lichens, algae and protists recorded in waarnemingen.be in Flanders, Belgium") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
...
```{r}
write_csv(occurrence, here::here("datasets", "np-datavragen-data-mossen-korstmossen", "data", "processed", "occurrence_mosses_etc.csv"), na = "")
```
