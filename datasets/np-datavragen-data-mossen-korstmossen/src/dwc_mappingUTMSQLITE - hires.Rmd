...
subtitle: "VBP-INBO-waarnemingen-occurrences_202_mossen_wieren_hoge resolutie
...
```{r eval=FALSE, include=FALSE}
input_interim <- dbReadTable(con, "INBO_WNMN_MOSSES_LICHENS_PROTISTS_from2019")  # werkt ook voor views
```
...
input_interim <- input_interim %>% 
  mutate(
    scientificName = scientificName %>% 
      str_remove("\\s+(spec|indet)\\.?$") %>%  # verwijdert ' spec.', ' spec', ' indet.', ' indet'
      str_trim()
  )
...
```{r}
occurrence %<>% mutate(dwc_datasetID = "https://doi.org/10.15468/ju9z3p")
```
...
```{r}
occurrence %<>% mutate(dwc_datasetName = "INBO collaborator Occurrences -High resolution - of Bryophyta, lichens, algae and protists recorded in waarnemingen.be in Flanders, Belgium") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
...
### kingdom (created by GBIF in later function)
...
```{r}
write_csv(occurrence_joined, here::here("datasets", "np-datavragen-data-mossen-korstmossen", "data", "processed", "occurrence_mosses_etc_high_res.csv"), na = "")
```
