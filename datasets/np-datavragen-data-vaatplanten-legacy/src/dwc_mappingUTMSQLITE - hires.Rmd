...
subtitle: "VBP-INBO-waarnemingen-occurrences_planten-legacy
...
```{r eval=FALSE, include=FALSE}
input_interim <- dbReadTable(con, "INBO_WNMN_PLANTS_legacy")  # werkt ook voor views
```
...
```{r}
occurrence %<>% mutate(dwc_datasetName = "INBO collaborator Occurrences -High resolution - of plants recorded in waarnemingen.be in Flanders, before 2018, Belgium") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
...
```{r}
write_csv(occurrence, here::here("datasets", "np-datavragen-data-vaatplanten-legacy", "data", "processed", "occurrence_planten_high_res.csv"), na = "")
```
