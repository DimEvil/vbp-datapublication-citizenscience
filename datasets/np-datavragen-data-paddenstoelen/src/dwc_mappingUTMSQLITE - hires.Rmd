...
subtitle: "VBP-INBO-waarnemingen-occurrences_2026_paddestoelen_hoge resolutie
...
```{r eval=FALSE, include=FALSE}
input_interim <- dbReadTable(con, "INBO_WNMN_FUNGI_from2019")  # werkt ook voor views
```
...
```{r}
occurrence %<>% mutate(dwc_datasetID = "https://doi.org/10.15468/ujjjq2")
```
...
```{r}
occurrence %<>% mutate(dwc_datasetName = "INBO collaborator Occurrences -High resolution - of fungi recorded in waarnemingen.be in Flanders, Belgium") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
...
```{r}
occurrence %<>% mutate(dwc_kingdom = "Fungi")
```
...
```{r}
write_csv(occurrence_joined, here::here("datasets", "np-datavragen-data-paddenstoelen", "data", "processed", "occurrence_fungi_high_res.csv"), na = "")
```
