...
subtitle: "VBP-INBO-waarnemingen-occurrences_2024_ongewervelden_hoge resolutie
...
```{r eval=FALSE, include=FALSE}
input_interim <- dbReadTable(con, "INBO_WNMN_INVERTEBRATA_from2019")  # werkt ook voor views
```
...
```{r}
occurrence %<>% mutate(dwc_datasetID = "https://doi.org/10.15468/kxsxk9")
```
...
```{r}
occurrence %<>% mutate(dwc_datasetName = "INBO collaborator Occurrences -High resolution - of invertebrates recorded in waarnemingen.be in Flanders, Belgium") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
...
```{r}
write_csv(occurrence_joined, here::here("datasets", "np-datavragen-data-ongewervelden", "data", "processed", "occurrence_invertebrates_high_res.csv"), na = "")
```
